...
\chapter{Bringing 8-bit projects to the market: Case studies of similar projects}
\label{Chapter3}
This chapter attempts to form a body of knowledge on retro computer projects. The work is focused on the process of getting the product to market and highlighting the potential risks challenges that can arise in that context. It also looks further into a products timeline where possible, such as with the Arduino and Raspberry Pi case studies. This chapter outlines challenges that where faced by the teams behind the Spectrum Vega, Spectrum Vega+, Spectrum Next, Raspberry Pi, Arduino, C64DTV and C64Mini. These challenges where uncovered during case studies into these projects. For each case study there is a brief description of the product, the team behind it and any information about the productisation process or challenges faced during that process, is recorded. From this record a list of the process steps is extracted as well as a list of challenges the project faced. The challenges are then categorised into risk types for each case study. Finally at the end of the chapter the separate case study processes are combined into a list and then illustrated in figure \ref{case_study_process}. The separate case study risks are combined into a risk table and ranked following the guidelines of \textit{AS/ISO 31000:2018 Risk management — Guidelines} 
\cite{RN164}. The risk was determined using the following definitions of harm levels:
\begin{enumerate}
...
\end{enumerate}
and the following risk matrix was used to determine the risk levels. The one significant way in which the risk assessment of the case study challenges is different from a normal risk assessment, is that normally the likelihood is a measure of probability of an event happening, but as the events in the case study have already happened, that uncertainty is removed. To account for this, the likelihood used in the risk assessments for the case studies is an estimate of how likely the event would have been at the start of the respective project.

\begin{figure} \begin{center}
\includegraphics[width=.9\linewidth]{pics/riskmatrix} 
\end{center} 
\caption{Risk matrix used in risk assessment}
...
\item Legal challenges to ownership or "Arduino" trademark
...
\textbf{When was it produced?}\\
The Raspberry Pi version 1 Model B was first sold in February, 2012 for \$35, with Model A coming out soon after for \$25 
\cite{RN100}.\\
...
\section{Combined Process}
This section consolidates the ad hoc processes collected from the case studies above. A discussion of any interesting or useful conclusions drawn from the comparison follows.
...
\section{Combined Risks}
The risk identified in the case studies are combined and ranked to produce a list of risks to consider that are more specific to a retro computer project. It is intended these risks be considered along side the typical project risks, such as time, money and technical ability, as examples. 
